...
const MarkerGridBlue = ({ markers }) => {
  ...
  const groupedMarkers = markers.reduce((acc, test) => {
    const { city } = test;
    if (!acc[city]) {
      acc[city] = [];
    }
  ...
  return (
    ...
    <div className="border-2 rounded-lg border-dgreen dark:border-lblue p-4 -mx-16 sm:-mx-16 md:-mx-16 lg:-mx-0 xl:-mx-0 z-20 -translate-y-[800px] -translate-x-60 sm:-translate-x-48 md:-translate-x-36 lg:-translate-x-0 xl:-translate-x-0">
    <CitySearchBox cities={Object.keys(groupedMarkers)} onCitySearch={handleCitySearch} />
    {Object.keys(groupedMarkers).map((city, index) => (
  <div key={city} className="border rounded-lg border-dgreen dark:border-lblue p-8 mb-4 " ref={(el) => (cityRefs[city] = el)}>
          ...
          {groupedMarkers[city].map((location, locationIndex) => (
            ...
            >
              <TbCurrentLocation size={28} className='dark:block hidden text-lblue' />
              <BiCurrentLocation size={28} className='dark:hidden block text-lblue' />
            </button>
          </div>
          ))}
        </div>
